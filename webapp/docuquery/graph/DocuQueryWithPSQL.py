...
from docuquery.graph.PostgresRetriever import PostgresRetriever
...
def generate_answer(state):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an intelligent assistant that provides answers based on the given contextual data."),
        ("human", """
                Given the following input:
                - Query: {query}
                - Postgres Rows: {postgres_rows}
    ...
    postgres_rows = state.get("postgres_rows")
...
def retrieve_psql_data(state):
    """
        Retrieve rows from postgres DB

        Args:
            state (dict): The current graph state

        Returns:
            state (dict): New key added to state, postgres_rows, that contains retrieved postgres data
        """

    print("---POSTGRES RETRIEVE---")
    ...
    data = []
    try:
        # data = chain.invoke({"question": query})
        # data = PostgresRetriever.to_list(data)
        data = []
    except Exception as error:
        print(f'Error retrieving data from Postgres DB: {str(error)}')
    return {"postgres_rows": data}
...
class DocuQuery:
    ...
    def get_graph():
        ...
        workflow.add_node("retrieve_psql_data", retrieve_psql_data)
        workflow.add_node("relevancy_check", relevancy_check)
        workflow.add_node("retrieve_documents", retrieve_documents)

        # Build graph
        workflow.set_entry_point("retrieve_documents")
        workflow.add_edge("retrieve_documents", "retrieve_psql_data")
        workflow.add_edge("retrieve_psql_data", "permission_check")
